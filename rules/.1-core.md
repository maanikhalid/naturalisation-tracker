...
**Plan before code:** For complex multi-step features (e.g. new API module + tenant UI + seed), write a concise plan (recon digest + objectives + impact + strategy) before coding. Execute in plan order. This reduces rework and keeps API, DB, and UI in sync.
...
**Don't narrow scope without an explicit user request.** When the user describes a UI element "with X" (e.g. "dropdown with the avatar") and the current UI already shows X plus more (e.g. avatar, org name, email), keep the full context unless they explicitly say "only X" or "avatar only". Interpreting "with the avatar" as "trigger = avatar only" over-narrows and causes rework.

**When the user corrects scope** (e.g. "I want the Org and email there as well"): You over-narrowed; restore the full requested scope. Treat this like a search correction: acknowledge, fix, and apply the lesson next time.
