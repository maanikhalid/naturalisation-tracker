#!/usr/bin/env node
/**
 * Undo an apply run from close-off-december-pending.mjs using its backup JSON.
 *
 * Usage (from web/):
...
async function main() {
  ...
  if (
    !backup ||
    backup.kind !== "december-2025-closeoff-backup" ||
    !Array.isArray(backup.rows)
  ) {
    console.error("Invalid backup format for December closeoff undo.");
    process.exit(1);
  }
