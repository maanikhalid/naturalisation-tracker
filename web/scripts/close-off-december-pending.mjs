#!/usr/bin/env node
/**
 * Close off 90% of pending December 2025 applications by assigning:
 * - approvalDate in range 2026-04-09 .. 2026-04-23
 ...
 *
 * Usage (from web/):
 *   node scripts/close-off-december-pending.mjs --dry-run
 *   node scripts/close-off-december-pending.mjs --apply
 *
 * Optional:
 *   --backup-dir ./scripts/backups
 */
...
const APPROVAL_START = "2026-04-09";
...
async function main() {
  ...
  const backupDirArg = argValue("--backup-dir");
  const backupDir = backupDirArg
    ? path.resolve(process.cwd(), backupDirArg)
    : path.resolve(process.cwd(), "scripts/backups");
  ...
  try {
    const pendingDecember = await prisma.timelineEntry.findMany({
      where: {
        isRemoved: false,
        approvalDate: null,
        applicationDate: {
    ...
    const pendingCount = pendingDecember.length;
    const targetCount = Math.floor(pendingCount * TARGET_RATIO);
    const selected = shuffle(pendingDecember).slice(0, targetCount);
    ...
    if (apply) {
      ...
      backupPath = path.join(backupDir, `dec-2025-closeoff-backup-${stamp}.json`);

      const backupPayload = {
        kind: "december-2025-closeoff-backup",
    ...
    const websitePending = pendingDecember.filter((r) => r.sourceType === "WEBSITE").length;
    const redditPending = pendingDecember.filter((r) => r.sourceType === "REDDIT").length;
      ...
      JSON.stringify(
        {
          ...
          counts: {
            pendingDecemberTotal: pendingCount,
            pendingDecemberWebsite: websitePending,
            pendingDecemberReddit: redditPending,
