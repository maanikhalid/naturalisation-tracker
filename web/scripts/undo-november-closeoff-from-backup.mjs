#!/usr/bin/env node
/**
 * Undo an apply run from close-off-november-pending.mjs using its backup JSON.
 *
 * Usage (from web/):
...
async function main() {
  ...
  if (
    !backup ||
    backup.kind !== "november-2025-closeoff-backup" ||
    !Array.isArray(backup.rows)
  ) {
    console.error("Invalid backup format for November closeoff undo.");
    process.exit(1);
  }
